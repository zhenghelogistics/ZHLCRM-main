...
// Initialize a separate Firebase app instance for the seeder if needed, or reuse the main one.
const app = initializeApp(firebaseConfig);
const db = getFirestore(app);

// --- HELPER TO CREATE DATES ---
// To make the demo timeless, we base all dates off the current date.
const now = new Date();
...
const currentMonthLeads: Omit<Lead, 'id'>[] = [
    // --- Active Leads for Pipeline ---
    {
        ...
        stage: "T-24h (Nurture)",
        createdAt: dateInCurrentMonth(today.getDate() - 1, 0), // Yesterday
        next_follow_up: dateInCurrentMonth(today.getDate(), 1), // Due in 1 hour
        riskLevel: RiskLevel.LOW,
        notes: "High potential deal for server rack transport."
    },
    {
        customer_name: "BioGen Pharma", // This customer ghosted in June
        quoted_price: 7800,
        industry: "Pharmaceuticals",
        lead_score: 8,
        status: LeadStatus.QUOTE_SENT,
        stage: "T+6h",
        createdAt: dateInCurrentMonth(today.getDate(), 4), // 4 hours ago
        next_follow_up: dateInCurrentMonth(today.getDate(), -2), // Due in 2 hours
        riskLevel: RiskLevel.MEDIUM,
        notes: "Previous ghosting history. Follow up carefully."
    },
    {
        customer_name: "Global Textiles Inc.",
        quoted_price: 4200,
        industry: "Manufacturing",
        lead_score: 6,
        status: LeadStatus.QUOTE_SENT,
        stage: "T+3h",
        createdAt: dateInCurrentMonth(today.getDate(), 1), // 1 hour ago
        next_follow_up: dateInCurrentMonth(today.getDate(), -2), // Due in 2 hours
        riskLevel: RiskLevel.LOW,
    },
    {
        customer_name: "Quantum Computing Co.",
        quoted_price: 25000,
        industry: "Tech",
        lead_score: 10,
        status: LeadStatus.IN_DISCUSSION,
        stage: "Follow-up Call",
        createdAt: dateInCurrentMonth(today.getDate() - 3),
        next_follow_up: dateInCurrentMonth(today.getDate() + 1), // Tomorrow
        riskLevel: RiskLevel.LOW,
        respondedAtStage: "T-24h (Nurture)",
        notes: "Responded positively. Scheduled a call to finalize details."
    },
    // --- Recently Closed Leads (for dashboard cards) ---
    {
        customer_name: "Dynamic Retail",
        quoted_price: 8900,
        industry: "Retail",
        lead_score: 7,
        status: LeadStatus.WON,
        stage: "Closed",
        createdAt: dateInCurrentMonth(today.getDate() - 2),
        next_follow_up: dateInCurrentMonth(today.getDate() - 2),
        riskLevel: RiskLevel.LOW,
        respondedAtStage: "T+6h",
        notes: "Deal closed after offering a small discount."
    },
    {
        customer_name: "Fusion Foods",
        quoted_price: 3200,
        industry: "Commodities",
        lead_score: 5,
        status: LeadStatus.LOST_REJECTED,
        stage: "Closed",
        createdAt: dateInCurrentMonth(today.getDate() - 4),
        next_follow_up: dateInCurrentMonth(today.getDate() - 4),
        riskLevel: RiskLevel.LOW,
        respondedAtStage: "T+3h",
        notes: "Client found a cheaper alternative with a local provider."
    },
];

const julyArchive: MonthlyArchive = {
    id: `${today.getFullYear()}-${String(today.getMonth()).padStart(2, '0')}`, // Last month
...
const juneArchive: MonthlyArchive = {
    id: `${today.getFullYear()}-${String(today.getMonth() - 1).padStart(2, '0')}`, // 2 months ago
...
export const seedDatabaseWithSampleData = async () => {
    try {
        ...
        // Add current month's leads to the 'leads' collection
        currentMonthLeads.forEach((lead, index) => {
            const newLeadRef = doc(leadsCollectionRef); // Auto-generate ID
            batch.set(newLeadRef, lead);
        });
        
        // Add archives to the 'archives' collection
