...
export function parseQuoteEmail(rawText: string): ParsedQuote {
    ...
    try {
        ...
        // --- Mode ---
        if (lowerText.includes('by sea') || lowerText.includes('ocean freight') || lowerText.includes('lcl') || lowerText.includes('fcl')) {
            result.mode = 'Sea';
        ...
        if (result.total_price === 0) {
            ...
            for (const line of lines) {
                ...
                // Check for the specific "(USD xx)" format first.
                const bracketMatch = line.match(/\(USD\s*([\d,]+\.?\d*)\)/i);
                if (bracketMatch && bracketMatch[1]) {
                    value = parseFloat(bracketMatch[1].replace(/,/g, ''));
                    if (!isNaN(value)) {
                        calculatedSum += value;
                        pricesFound = true;
                        continue; // This line is handled, move to the next.
                    }
                }

                // If no bracket format, find all "USD xx" and take the LAST one.
                // This correctly handles lines with calculations like "USD A x B = USD C" by taking C.
                const allMatches = Array.from(line.matchAll(/(?:USD|\$)\s*([\d,]+\.?\d*)/gi));
                if (allMatches.length > 0) {
                    const lastMatch = allMatches[allMatches.length - 1];
                    if (lastMatch && lastMatch[1]) {
                        value = parseFloat(lastMatch[1].replace(/,/g, ''));
                        if (!isNaN(value)) {
                            calculatedSum += value;
                            pricesFound = true;
                        }
