...
const FilterButton: React.FC<{ label: string, filter: FilterOption, activeFilter: FilterOption, onClick: (filter: FilterOption) => void }> = ({ label, filter, activeFilter, onClick }) => (
    <button 
        ...
        className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
            ...
            : 'bg-zinc-800/50 text-slate-200 hover:bg-zinc-700/50 border border-zinc-700'
        }`}
    >
        {label}
    </button>
    ...
    return (
        <td className="py-4 px-3 text-sm min-w-[200px]">
            <textarea
                ...
                className="w-full bg-transparent border border-transparent hover:border-slate-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-md p-1 text-slate-300 text-xs resize-none placeholder:text-slate-500"
                rows={1}
            />
        </td>
    );
    ...
    return (
        <td className="py-2 px-3 text-sm">
            <input
                ...
                className="w-full bg-transparent border border-transparent hover:border-slate-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-md p-1 text-slate-300"
...
const ReportingPage: React.FC<ReportingPageProps> = ({ allLeads, onAddLead, onUpdateNotes, onDelete, onUpdateDate }) => {
    ...
    const filteredLeads = useMemo(() => {
        ...
        if (activeFilter !== 'all') {
            leads = leads.filter(lead => {
                switch (activeFilter) {
                    case 'responded':
                        return lead.status === LeadStatus.IN_DISCUSSION;
                    case 'won':
                        return lead.status === LeadStatus.WON;
                    case 'ghosted':
                        return lead.status === LeadStatus.LOST_GHOSTED;
                    case 'rejected':
                        return lead.status === LeadStatus.LOST_REJECTED;
                    case 'T+3h':
                    case 'T+6h':
                    case 'T-24h (Nurture)':
                        return lead.stage === activeFilter && lead.status === LeadStatus.QUOTE_SENT;
                    default:
                        return true;
                }
            });
    ...
    const handleExportCSV = () => {
        ...
        const rows = allLeads.map(lead => [lead.id, lead.customer_name, lead.quoted_price, lead.industry, lead.lead_score, lead.status, lead.stage, lead.respondedAtStage, lead.riskLevel, new Date(lead.createdAt).toISOString(), lead.notes].map(escapeCSV).join(','));
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-2xl font-bold text-slate-100">Reporting & Analysis</h1>
                    <p className="text-slate-400 mt-1">A complete view of all leads. Filter by stage or status to analyze performance.</p>
                </div>
                <button onClick={handleExportCSV} className="flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition">
            ...
            <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800/50 rounded-xl p-4">
                <div className="flex items-center text-slate-300">
                    ...
                    <p>
                        Showing <span className="font-bold text-slate-100">{summary.count} {summary.count === 1 ? 'lead' : 'leads'}</span>
                        {industryFilter !== 'all' ? ` in the "${industryFilter}" industry` : ''}
                        &nbsp;under "{filterMetadata[activeFilter].title}"
                        with a total value of <span className="font-bold text-slate-100">{formatCurrency(summary.totalValue)}</span>.
                    </p>
                </div>
            </div>
                ...
                <FilterButton label="Ghosted" filter="ghosted" activeFilter={activeFilter} onClick={setActiveFilter} />
                 <div className="relative">
                    <select
                        ...
                        className="appearance-none bg-zinc-800/50 border border-zinc-700 text-slate-200 text-sm rounded-md py-1.5 pl-3 pr-8 hover:bg-zinc-700/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                    >
            ...
            <div className="bg-zinc-900/40 backdrop-blur-md border border-zinc-800/50 rounded-xl shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-700">
                        <thead className="bg-slate-700/50">
                            <tr>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200 sm:pl-6">Customer</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Score</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Price</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Final Status</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Responded At</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Created</th>
                                <th className="py-3.5 px-3 text-left text-sm font-semibold text-slate-200">Notes</th>
                        ...
                        <tbody className="divide-y divide-slate-700 bg-slate-800">
                            {filteredLeads.map(lead => (
                                <tr key={lead.id} className="hover:bg-slate-700/50">
                                    ...
                                    <td className="py-4 px-3 text-sm text-slate-200 font-bold text-center">{lead.lead_score}</td>
                                    <td className="py-4 px-3 text-sm text-slate-300">{formatCurrency(lead.quoted_price)}</td>
                                    <td className="py-4 px-3 text-sm">
                                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                            lead.status === LeadStatus.IN_DISCUSSION ? 'bg-teal-500/20 text-teal-300' :
                                            lead.status === LeadStatus.WON ? 'bg-green-500/20 text-green-300' :
                                            lead.status === LeadStatus.LOST_GHOSTED ? 'bg-red-500/20 text-red-300' :
                                            lead.status === LeadStatus.LOST_REJECTED ? 'bg-slate-500/20 text-slate-300' :
                                            'bg-blue-500/20 text-blue-300'
                                        }`}>
                                            {lead.status}
                                        </span>
                                    </td>
                                    <td className="py-4 px-3 text-sm text-slate-400">{lead.respondedAtStage || 'N/A'}</td>
             ...
             {filteredLeads.length === 0 && (
                <div className="text-center py-12 border-2 border-dashed border-zinc-800/50 rounded-xl">
                    <p className="text-sm text-slate-400">No leads match the current filter.</p>
                </div>
            )}
        </div>
    );
