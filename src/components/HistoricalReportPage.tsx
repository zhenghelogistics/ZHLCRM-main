...
const HistoricalReportPage: React.FC<HistoricalReportPageProps> = ({ archives }) => {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-800">Historical Analysis</h1>
                <p className="text-slate-500 mt-1">Review performance from previous months.</p>
            </div>
            ...
            {archives.length > 0 ? (
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    <div className="lg:col-span-1">
                        <div className="bg-white/80 backdrop-blur-md border border-slate-200 rounded-xl p-4 shadow-sm">
                            <h2 className="font-semibold text-slate-700 mb-3">Select a Month</h2>
                        ...
                        {selectedArchive ? (
                            <>
                                <h2 className="text-xl font-bold text-slate-700">
                                    Report for: <span className="text-blue-600">{selectedArchive.displayName}</span>
                                </h2>
                ...
                <div className="text-center py-16 bg-white/80 backdrop-blur-md rounded-xl shadow-sm border-2 border-dashed border-slate-200">
                    <HistoryIcon className="mx-auto h-12 w-12 text-slate-400" />
                    <h3 className="mt-2 text-sm font-medium text-slate-700">No Archives Found</h3>
                    <p className="mt-1 text-sm text-slate-500">Archive a month from the main pipeline dashboard to start building your history.</p>
                </div>
            )}
        </div>
    );
