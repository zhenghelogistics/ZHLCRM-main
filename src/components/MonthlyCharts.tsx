...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-slate-200 p-2 rounded-md shadow-lg">
        <p className="label text-slate-900 font-medium">{`${label}`}</p>
...
const MonthlyCharts: React.FC<MonthlyChartsProps> = ({ leads }) => {
    ...
    const COLORS = ['#ef4444', '#94a3b8']; // Red-500, Slate-400
    const textAndGridColor = '#64748b'; // slate-500 for better visibility on white
